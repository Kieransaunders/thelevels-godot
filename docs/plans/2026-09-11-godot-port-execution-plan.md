...
- [x] GP1 (detour between P4 and P5, user-directed): wetland art pass — stylized water + atmosphere. Completed 2026-09-12: water shader rebuilt (per-pixel depth-texture thickness, Beer-Lambert absorption to slate-blue, screen-space refraction with depth-mask ghost fix, two-octave scrolling detail normals with distance-scaled epsilon, waterline foam + noise belt); terrain recoloured to hard elevation bands from the Somerset concept with lowland-weighted reed tint and wet shoreline sheen; environment moved to AgX tonemap, dawn sun pitch, dense-ish warm fog, SSAO, glow, saturation adjustment. Rendering-only: no solver, mesh-topology or COLOR.a contract changes. ./verify.sh green (26/26, P3+P4 adapter checks), before/after captures in docs/verification/. Flow whitewater, caustics, ripple simulation deferred pending need.
...
- [x] P6: native tool/ritual VFX and permitted asset provenance. Completed 2026-09-12: 35/35 green, --verify-p6 adapter gate green (buffer-driven god hand: empty/half/full/zero-capacity, hidden for lightning; tool + bolt + ritual effects; seven configured tube-trail rises; P4 bolt cylinder removed; flow/depth vertex channels). God hand, spell feedback and ritual are native GPUParticles/StandardMaterial3D work; only the three CC0 textures in Assets/Vfx ship (provenance in Assets/Vfx/LICENSE.md; GDQuest NC art untouched, zero dependency on reference/). Deliberate ADR exception: read-only HeightfieldSimulation.FlowSpeed presentation accessor (contract-tested) feeding the deferred water extras — flow whitewater and submerged caustics implemented; ripple simulation stays rejected. Evidence: docs/verification/2026-09-11-p0.md §P6 and five 2026-09-12-p6-*.png captures.
- [x] P7: integration hardening, profiling and release candidate. Completed 2026-09-12: 38/38 green (new 50-action accounting, channel/bank/overtopping and 600 s soak tests with full mass reconciliation via the new WaterLossVolume diagnostic), --verify-p7 adapter gate green (reset cycles, paused editing, step durations, corner/border brushing, domain-safe targeting, VFX reload cleanup). Profiled at 1920×1080 on Apple M4: 124 FPS, water step 1.63 ms, mesh rebuild 8.46 ms, zero corrections. Standalone via Release build + `Play The Levels.command` launcher (signed .app export deferred — templates not installed). Known defects and deviations consolidated in docs/verification/2026-09-11-p0.md §P7. Tag port-v0.1.
- [ ] P8 (optional, user-directed): polish backlog discovered during play.
