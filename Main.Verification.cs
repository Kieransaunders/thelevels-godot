...
// Headless verification gates (--verify-p3..p7, --verify-input) and the --showcase and
// --capture routines. Split out of Main.cs so scene setup and the checks that exercise it
// stop sharing one file; partial class, so no call site changed.
// ponytail: these need a live scene tree (view, camera, cursor, VFX), so they cannot move
// to Tests/ — that project is NUnit against engine-free Core/ only.
public partial class Main
{
    private void VerifyView(SimulationHost host, HeightfieldView view, Diagnostics diagnostics)
    {
        ...
        // Sim corners are exact water-grid vertices (stride WaterSubdiv); the bilinear
        // sub-vertices in between may carry fractional depth — that is the smooth shore.
        ...
        for (int x = 0; x < host.Heightfield.Resolution; x++)
        {
            ...
            float alpha = colors[x * HeightfieldView.WaterSubdiv + z * HeightfieldView.WaterSubdiv * view.WaterGrid].A;
    ...
    /// <summary>Drives a synthesized W keypress and trackpad pan-scroll through the input pipeline and checks the camera responds to both.</summary>
    private async void VerifyInput(StrategyCamera camera, Diagnostics diagnostics)
    {
        ...
        // macOS trackpads deliver scroll as InputEventPanGesture, never wheel buttons;
        // -20 px is a swipe up, which must zoom in like WheelUp.
        float zoomBefore = camera.Distance;
        Input.ParseInputEvent(new InputEventPanGesture { Delta = new Vector2(0f, -20f) });
        for (int i = 0; i < 3; i++) await ToSignal(GetTree(), SceneTree.SignalName.ProcessFrame);
        float zoomAfter = camera.Distance;
        bool zoomed = zoomAfter < zoomBefore;

        GD.Print($"INPUT CHECK  pressed={Input.IsActionPressed(InputBindings.CamForward)}  before[{before}]  after[{after}]  moved={moved}  panZoom {zoomBefore:0.0}->{zoomAfter:0.0}  zoomed={zoomed}");
        GD.Print(diagnostics.Snapshot);
        GetTree().Quit(moved && zoomed ? 0 : 1);
    }
